...
const GENERIC_STOCK = ["photo-1616486338812-3dadae4b4ace"];
...
export default async function HomePage() {
  ...
  const sections = ensureHomeSections(catalog);
  const latest = [...catalog.products].sort(
    (a, b) =>
      new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  );

  const categories = resolveHomeCategories(catalog);
  ...
  const featured = resolveHomeProducts(catalog, "featured", featuredFallback, 4);
  const trending = resolveHomeProducts(catalog, "trending", trendingFallback, 4);
  const bestsellers = resolveHomeProducts(
    catalog,
    "bestsellers",
    bestsellersFallback,
    4
  ...
  const artisanSource = resolveHomeProducts(
    catalog,
    "artisans",
  ...
  const atelierImages = (
    sections.atelier.itemIds.length
      ? atelierProducts.flatMap((p) => p.images.slice(0, 2))
      : [
          ...latest.flatMap((p) => p.images.slice(0, 2)),
          ...categories.map((c) =>
            categoryDisplayImage(c, productImageByCategory)
          ),
        ]
  )
    .filter(Boolean)
  ...
  while (atelierImages.length < 6 && sections.atelier.visible) {
    atelierImages.push(
  ...
  return (
    <div>
      ...
      {sections.collections.visible && categories.length > 0 && (
        <section
          id="collections"
      ...
      {sections.artisans.visible && artisans.length > 0 && (
        <section
          id="artisans"
          className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8"
        >
